...
import { apiV1BaseUrl, requestErrorMessage } from '../api/client'
...
describe('API URL normalization', () => {
  ...
  it('returns a structured server error message', () => {
    expect(
      requestErrorMessage({
        isAxiosError: true,
        response: { data: { detail: { message: 'Choose an active program.' } } },
      })
    ).toBe('Choose an active program.')
  })
})
