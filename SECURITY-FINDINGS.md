...
| **Authentication / Session management** | Runtime: N/A; Prototype: **IDOR documented** | No auth in `src/`. Prototype's `/recommendations/{reviewer_id}/*` endpoints accept `reviewer_id` from the URL with no auth check: a classic IDOR. See §4.2. |
