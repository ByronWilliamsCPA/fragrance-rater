...
| Operational events/status | Recent failures, recoveries, and affected evaluator names | Manager reads are `403`; assigned recorders can only append reviewer-scoped events | `private, no-store`; status contains operational guidance without hostnames, credentials, or exception text | Failure/recovery API test |
...
| Error response | Actionable validation or authorization message | No mapping, version identity, secret, or infrastructure detail | Errors are generated after authorization checks; operational status uses fixed guidance | API lifecycle and UI retry tests |
