...
ADR-017 generalizes `provider_term_mapping`'s target (introduced in the 2026-09-15 amendment
...
`vocabulary_term`) under ADR-017's four-layer intake model. `provider_key` values now include
`house:<brand_id>` (a specific niche house), `evaluator`, and `edwards`, alongside the existing
`fragella` and crosswalk providers. Candidate discovery, scoring, and Fragella's storage
boundary above are unchanged. See ADR-017 for the full model.
...
- [ADR-017](./adr-017-project-owned-faceted-olfactory-vocabulary.md): generalizes
