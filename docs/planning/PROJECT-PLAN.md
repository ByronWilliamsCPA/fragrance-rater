...
| Parfumo capture | Deprecated (ADR-012, 2026-09-15) | `ParfumoScraper` and its CLI commands are being retired; no new writes use this path (retirement tracked as R9). P1.9's fixtures and parser coverage are retained as evidence of pre-deprecation work only. Baseline/holdout facts sourced solely from Parfumo are being re-verified against manufacturer confirmation and Wikidata per ADR-012 Decision 5; `SourceSnapshot`'s ADR-012 provenance columns (`source_type`, `permission_state`, `fields`, `source_reference`) are tracked as R8 |
