...
| --- | --- | --- | --- | --- | --- |
| `/ratings` | `RatingsPage` | Covered: exercised via `src/test/App.test.tsx` (encounter creation, correction, worn-by selection and clearing), 40/40 suite passing | Covered: `e2e/ordinary-entry.spec.ts` passing | Not run in CI/sandbox; requires live Docker backend, and this route is not exercised by the smoke spec even when a backend is available (see note above) | Covered: `e2e/accessibility.spec.ts` (`/ratings` case) passing, zero axe violations |
