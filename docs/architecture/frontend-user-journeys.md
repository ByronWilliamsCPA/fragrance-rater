...
| --- | --- | --- | --- | --- | --- |
| `/ratings` | `RatingsPage` | Covered: exercised via `src/test/App.test.tsx` (encounter creation, correction, worn-by selection and clearing), 40/40 suite passing | Partially covered: `e2e/ordinary-entry.spec.ts` covers creation only; the correction form (`RatingsPage.tsx:213-259`) is covered at the Vitest tier only | Never wired into CI; the sandbox attempt above was blocked before any assertion ran (`ECONNREFUSED`, no backend), and this route is not exercised by the smoke spec even when a backend is available (see note above) | Covered: `e2e/accessibility.spec.ts` (`/ratings` case) passing, zero axe violations |
