...
def upgrade() -> None:
    """Add legacy constraints without deleting data.

    Safety patch: older copies deleted catalog versions and encounter history.
    The unique constraint now fails atomically when duplicates exist. Such an
    older database requires preservation-aware reconciliation before upgrade;
    never delete ratings merely to satisfy this superseded uniqueness policy.
    Databases that already applied this revision are unaffected.
    """
...
def downgrade() -> None:
    """Remove legacy constraints; cannot recover deletions by older code."""
    op.drop_constraint(
